"""Comment command handler - creates comments on user profiles."""
import discord
...
async def handle_comment_command(discord_id: str, username: str, target_discord_id: str, comment_text: str) -> discord.Embed:
    """Handle comment command - create a comment on a user's profile."""
    try:
        # Get auth token
        ...
        if not comment_text:
            return create_usage_embed(
                f"Comment cannot be empty.\n\n**Usage:** `{PREFIX}comment @user <comment>`\n**Slash:** `/comment user:<user> comment:<comment>`",
                examples=f"{PREFIX}comment @john Great job on landing that role!\n{PREFIX}comment @jane Your process insights were really helpful."
            )
        
        if len(comment_text) > 2000:
            return create_error_embed(
                "Comment Too Long",
                "Comments must be 2000 characters or less."
            )
        ...
        return create_success_embed(
            "Comment Posted",
            f"Your comment has been posted to `{target_username}`'s profile.\n\n"
            f"[View Profile]({profile_url})"
        )
    except Exception as e:
        return handle_command_error(e, "posting comment")


def setup_comment_command(bot: commands.Bot):
    """Setup comment command (both slash and prefix)."""
    # Slash command
    @bot.tree.command(name="comment", description="Post a comment on someone's profile")
    @app_commands.describe(user="User to comment on", comment="Your comment")
    async def comment_command(interaction: discord.Interaction, user: discord.User, comment: str):
        """Post comment: /comment user:<user> comment:<comment>"""
        ...
        embed = await handle_comment_command(discord_id, username, target_discord_id, comment)
        await interaction.response.send_message(embed=embed)
    
    # Prefix command
    @bot.command(name="comment")
    async def comment_command_prefix(ctx: commands.Context, *, args: str = None):
        """Post comment: p!comment @user <comment>"""
        discord_id = str(ctx.author.id)
        username = ctx.author.name
        
        if not args:
            embed = create_usage_embed(
                f"**Usage:** `{PREFIX}comment @user <comment>`\n**Slash:** `/comment user:<user> comment:<comment>`",
                examples=f"{PREFIX}comment @john Great job on landing that role!\n{PREFIX}comment @jane Your process insights were really helpful."
            )
            await ctx.send(embed=embed)
            return
        
        # Parse arguments - expect @mention followed by comment
        args = args.strip()
        ...
        if ctx.message.mentions:
            ...
            comment_text = args.replace(f"<@{mentioned_user.id}>", "").replace(f"<@!{mentioned_user.id}>", "").strip()
        else:
            ...
            if match:
                ...
                comment_text = args[match.end():].strip()
            else:
                embed = create_usage_embed(
        ...
        embed = await handle_comment_command(discord_id, username, target_discord_id, comment_text)
        await ctx.send(embed=embed)
